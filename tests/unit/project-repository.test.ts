...
import type { CreateProjectData, UpdateProjectData, ProjectFilters } from '../../src/types/project';
...
describe('ProjectRepository', () => {
	...
	describe('findAll()', () => {
		beforeEach(async () => {
			...
			await testEnv.DB.batch([
				...
				).bind('PROJECT-101', '프로젝트1', '진행중', 'PERSON-001', '개발팀', '2025-01-01', '2025-06-30', now, now),
				testEnv.DB.prepare(
					`INSERT INTO projects (
						project_id, name, status, leader_person_id, dept_name, start_date, target_end_date, created_at, updated_at
					) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
				).bind('PROJECT-102', '프로젝트2', '완료', 'PERSON-002', '기획팀', '2025-02-01', '2025-07-31', now, now),
				testEnv.DB.prepare(
					`INSERT INTO projects (
						project_id, name, status, leader_person_id, dept_name, start_date, target_end_date, created_at, updated_at
					) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
				).bind('PROJECT-103', '프로젝트3', '보류', 'PERSON-001', '개발팀', '2025-03-01', '2025-08-31', now, now),
			]);
		});
		...
		it('should filter by leader person ID', async () => {
			...
			expect(result.map((p) => p.leaderPersonId)).toEqual(['PROJECT-103', 'PROJECT-101'].map(() => 'PERSON-001'));
		});
	...
	describe('addParticipant()', () => {
		...
		it('should throw ConflictError when adding duplicate participant', async () => {
			...
			// Act & Assert
			await expect(repository.addParticipant(projectId, 'PERSON-001')).rejects.toThrow(ConflictError);
		});
	});
	...
	describe('getStatistics()', () => {
		it('should return statistics for project with work notes and todos', async () => {
			...
			await testEnv.DB.batch([
				...
				).bind('FILE-001', 'PROJECT-STATS', 'projects/PROJECT-STATS/files/FILE-001', 'test.pdf', 'application/pdf', 1024, 'test@example.com', now),
				testEnv.DB.prepare(
					`INSERT INTO project_files (
						file_id, project_id, r2_key, original_name, file_type, file_size, uploaded_by, uploaded_at
					) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
				).bind('FILE-002', 'PROJECT-STATS', 'projects/PROJECT-STATS/files/FILE-002', 'test.png', 'image/png', 2048, 'test@example.com', now),
			]);
	...
	describe('getDetail()', () => {
		it('should return project detail with all associations', async () => {
			...
			await testEnv.DB.batch([
				...
				).bind('FILE-001', 'PROJECT-DETAIL', 'projects/PROJECT-DETAIL/files/FILE-001', 'test.pdf', 'application/pdf', 1024, 'test@example.com', now),
			]);
