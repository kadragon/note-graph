...
// Integration tests for work note to project association (Jest)
...
function mockEmbeddingFetch(): void {
  globalThis.fetch = jest.fn().mockImplementation(async (_input, init) => {
    ...
    const data = inputs.map((_: unknown, index: number) => ({
      embedding: [0, 0, 0],
      index,
    }));
...
describe('Work Note Project Association', () => {
  ...
  describe('POST /work-notes with projectId', () => {
    it('should create work note with project assignment', async () => {
      ...
      // Act
      const response = await authFetch(testEnv, 'http://localhost/api/work-notes', {
        method: 'POST',
  ...
  describe('GET /work-notes/:workId with projectId', () => {
    it('should return work note with projectId in detail', async () => {
      ...
      // Act
      const response = await authFetch(testEnv, `http://localhost/api/work-notes/${workId}`);
  ...
  describe('1:N Relationship Constraint', () => {
    it('should prevent assigning work note to second project via POST endpoint', async () => {
      ...
      const response = await authFetch(
        testEnv,
        'http://localhost/api/projects/PROJECT-B/work-notes',
        {
          method: 'POST',
  ...
  describe('Project Association Endpoints Consistency', () => {
    it('should maintain consistency between POST work-note and POST project/:id/work-notes', async () => {
      ...
      const createResponse = await authFetch(testEnv, 'http://localhost/api/work-notes', {
        method: 'POST',
        body: JSON.stringify({
          title: '업무1',
          contentRaw: '내용1',
      ...
      const listResponse = await authFetch(
        testEnv,
        'http://localhost/api/projects/PROJECT-001/work-notes'
      );
      expect(listResponse.status).toBe(200);
      const workNotes = (await listResponse.json()) as Array<{ workId: string }>;
