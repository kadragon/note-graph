// Trace: spec_id=SPEC-jest-migration-001 task_id=TASK-MIGRATE-001
...
describe('TodoRepository', () => {
  ...
  beforeEach(async () => {
    ...
    // Clean up test data
    await db.batch([db.prepare('DELETE FROM todos'), db.prepare('DELETE FROM work_notes')]);
  ...
  describe('findAll()', () => {
    ...
    it('should return all todos when view is "remaining"', async () => {
      // Act
      const result = await repository.findAll({ view: 'remaining' });

      // Assert
      // The 'remaining' view excludes future wait_until todos, so we expect at least 3
    ...
    it('should filter by status', async () => {
      // Act
      const result = await repository.findAll({ view: 'completed', status: '완료' });
