// Trace: spec_id=SPEC-stats-1 task_id=TASK-047 task_id=TASK-050 task_id=TASK-054 test_id=TEST-stats-1 test_id=TEST-stats-2 test_id=TEST-stats-3
/**
...
describe('StatisticsRepository', () => {
  ...
  beforeEach(async () => {
    ...
    await db.batch([
      ...
      db.prepare('DELETE FROM work_note_relation'),
      db.prepare('DELETE FROM work_note_task_category'),
      db.prepare('DELETE FROM todos'),
      db.prepare('DELETE FROM work_note_versions'),
      db.prepare('DELETE FROM work_notes'),
      db.prepare('DELETE FROM persons'),
      db.prepare('DELETE FROM departments'),
      db.prepare('DELETE FROM task_categories'),
    ]);
  });
  ...
  describe('findCompletedWorkNotes', () => {
    it('should return work notes with at least one completed todo', async () => {
      ...
      await db.batch([
        db
          .prepare(`INSERT INTO departments (dept_name, description) VALUES (?, ?)`)
          .bind('개발팀', 'Development'),
        db
          .prepare(
    ...
    it('should not return work notes with no completed todos', async () => {
      ...
      await db.batch([
        db
          ...
          .bind('개발팀', 'Development'),
        db
          .prepare(`INSERT INTO persons (person_id, name, current_dept) VALUES (?, ?, ?)`)
          .bind('P001', '홍길동', '개발팀'),
        db
          .prepare(
            `INSERT INTO work_notes (work_id, title, content_raw, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`
    ...
    it('should support filtering by category', async () => {
      ...
      await db.batch([
        ...
        db
          ...
          .bind('P001', '홍길동', '개발팀'),
        db
          .prepare(`INSERT INTO task_categories (category_id, name, created_at) VALUES (?, ?, ?)`)
        ...
        db
          .prepare(`INSERT INTO work_note_task_category (work_id, category_id) VALUES (?, ?)`)
    ...
    it('should count only todos completed within date range, not all historical completions', async () => {
      // Trace: spec_id=SPEC-stats-1 task_id=TASK-067 test_id=TEST-stats-7
