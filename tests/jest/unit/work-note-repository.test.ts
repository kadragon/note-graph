...
// Unit tests for WorkNoteRepository (Jest version)
...
describe('WorkNoteRepository', () => {
  ...
  describe('findByIdWithDetails()', () => {
    ...
    it('should find work note with person associations', async () => {
      ...
      await db.batch([
        db.prepare('INSERT INTO persons (person_id, name) VALUES (?, ?)').bind(personId, '홍길동'),
        db
          .prepare(
            'INSERT INTO work_notes (work_id, title, content_raw, category, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)'
          )
    ...
    it('should find work note with related work notes', async () => {
      ...
      await db.batch([
        ...
        db
          .prepare('INSERT INTO work_note_relation (work_id, related_work_id) VALUES (?, ?)')
  ...
  describe('findAll()', () => {
    ...
    it('should filter by department name', async () => {
      ...
      const deptName = '개발팀';
      await db.batch([
        db.prepare('INSERT INTO departments (dept_name) VALUES (?)').bind(deptName),
        db
  ...
  describe('create()', () => {
    ...
    it('should create work note with person associations', async () => {
      ...
      const personId = '123456';
      await db
        .prepare('INSERT INTO persons (person_id, name) VALUES (?, ?)')
        .bind(personId, '홍길동')
        .run();
    ...
    it('should create work note with multiple person associations', async () => {
      // Arrange
      await db.batch([
        db.prepare('INSERT INTO persons (person_id, name) VALUES (?, ?)').bind('P-001', 'Person 1'),
        db.prepare('INSERT INTO persons (person_id, name) VALUES (?, ?)').bind('P-002', 'Person 2'),
      ]);
  ...
  describe('update()', () => {
    ...
    it('should replace existing person associations', async () => {
      ...
      const update: UpdateWorkNoteInput = {
        persons: [{ personId: 'P-002', role: 'RELATED' }],
      };
  ...
  describe('delete()', () => {
    ...
    it('should cascade delete person associations', async () => {
      ...
      const associations = await db
        .prepare('SELECT * FROM work_note_person WHERE work_id = ?')
        .bind(created.workId)
        .all();
