...
(testEnv as any).R2_BUCKET = new MockR2() as unknown as R2Bucket;
...
describe('Project File Routes', () => {
	beforeEach(async () => {
		...
		(testEnv as any).R2_BUCKET = new MockR2() as unknown as R2Bucket;
		(globalThis as any).__TEST_R2_BUCKET = (testEnv as any).R2_BUCKET;
	...
	it('uploads, lists, downloads, and deletes a PDF file', async () => {
		...
		// Download
		const dlRes = await authFetch(`http://localhost/api/projects/PROJECT-FILE/files/${uploaded.fileId}/download`);
		expect(dlRes.status).toBe(200);
		expect(dlRes.headers.get('Content-Type')).toBe('application/pdf');
		expect(await dlRes.text()).toContain('hello pdf');

		// Delete
		const delRes = await authFetch(`http://localhost/api/projects/PROJECT-FILE/files/${uploaded.fileId}`, {
			method: 'DELETE',
		});
		expect(delRes.status).toBe(204);

		// Download after delete should 404
		const dlAfter = await authFetch(`http://localhost/api/projects/PROJECT-FILE/files/${uploaded.fileId}/download`);
