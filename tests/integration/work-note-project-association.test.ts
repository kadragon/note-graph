...
describe('Work Note Project Association', () => {
	...
	describe('PUT /work-notes/:workId with projectId', () => {
		...
		it('should assign work note to project via update', async () => {
			...
			const workNote_db = await testEnv.DB.prepare('SELECT project_id FROM work_notes WHERE work_id = ?')
				.bind(workId)
		...
		it('should change work note project assignment', async () => {
			...
			await testEnv.DB.batch([
				...
				testEnv.DB.prepare(`UPDATE work_notes SET project_id = ? WHERE work_id = ?`).bind('PROJECT-A', workId),
				testEnv.DB.prepare(
					`INSERT INTO project_work_notes (project_id, work_id, assigned_at) VALUES (?, ?, ?)`
		...
		it('should unassign work note from project', async () => {
			...
			await testEnv.DB.batch([
				...
				testEnv.DB.prepare(`UPDATE work_notes SET project_id = ? WHERE work_id = ?`).bind('PROJECT-001', workId),
				testEnv.DB.prepare(
					`INSERT INTO project_work_notes (project_id, work_id, assigned_at) VALUES (?, ?, ?)`
				).bind('PROJECT-001', workId, now),
			]);
			...
			const association = await testEnv.DB.prepare('SELECT * FROM project_work_notes WHERE work_id = ?')
				.bind(workId)
				.first();
