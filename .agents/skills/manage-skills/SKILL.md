...
| 스킬 | 설명 | 커버 파일 패턴 |
| --- | --- | --- |
| `verify-web-query-consistency` | 프론트엔드 React Query 키/무효화/API query-string 일관성 검증 | `apps/web/src/hooks/**/*.ts`, `apps/web/src/lib/{query-keys,query-invalidation,todo-repeat-rule,date-format,api}.ts`, `apps/web/src/pages/work-notes/components/view-work-note-dialog.tsx` |
| `verify-worker-route-consistency` | Worker 라우트 helper/NotFoundError/query schema 일관성 검증 | `apps/worker/src/routes/**/*`, `apps/worker/src/schemas/{admin,meeting-minute}.ts`, `tests/unit/{reembed-route-helper,schemas}.test.ts` |
