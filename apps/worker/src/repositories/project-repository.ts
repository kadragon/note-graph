...
export class ProjectRepository {
  ...
  async getFiles(projectId: string): Promise<ProjectFile[]> {
    ...
    return (results.results || []).map((r) => ({
      ...
      storageType: ((r.storage_type as string) || 'R2') as 'R2' | 'GDRIVE',
      gdriveFileId: (r.gdrive_file_id as string) || null,
      gdriveFolderId: (r.gdrive_folder_id as string) || null,
      gdriveWebViewLink: (r.gdrive_web_view_link as string) || null,
