...
describe('WeekCalendar', () => {
  ...
  it('starts the week on Sunday', () => {
    const { container } = render(
      <WeekCalendar events={[]} startDate={new Date('2026-01-19T00:00:00')} weeks={1} />
    );

    const headers = container.querySelectorAll('.bg-muted\\/50 > div');
    const labels = Array.from(headers).map((node) => node.textContent);
