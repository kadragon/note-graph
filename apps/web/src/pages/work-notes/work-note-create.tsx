...
export default function WorkNoteCreate() {
  ...
  const filteredMeetingMinutes = useMemo(() => {
    ...
    if (!keyword) return meetingMinutes;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedContent) {
      ...
      toast({ variant: 'destructive', title: '오류', description: '내용을 입력해주세요.' });
      return;
    }
    ...
    try {
      ...
      navigate(`/work-notes/${result.id}`, { replace: true });
    } catch {
    ...
    <div className="page-container">
      <div className="page-header">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="page-title">새 업무노트 작성</h1>
            <p className="page-description">새로운 업무노트를 작성합니다.</p>
          </div>
        </div>
      </div>
