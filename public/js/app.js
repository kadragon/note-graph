...
const App = {
          ...
          ${todos.map(todo => `
            <li class="todo-item">
              <input
                ...
                ${todo.status === '보류' || todo.status === '중단' ? 'disabled' : ''}
                title="${todo.status === '보류' || todo.status === '중단' ? '이 상태에서는 체크박스를 사용할 수 없습니다' : ''}"
              >
              <div class="todo-content">
                <div class="todo-title ${todo.status === '보류' || todo.status === '중단' ? 'todo-inactive' : ''}">${UI.escapeHtml(todo.title)}</div>
                <div class="todo-meta">
  ...
  async toggleTodoStatus(todoId, checked, checkboxElement) {
    // Safety check: only allow toggle for '진행중' and '완료' statuses
    const currentStatus = checkboxElement.dataset.status;

    // Prevent toggling for '보류' or '중단' statuses
    if (currentStatus === '보류' || currentStatus === '중단') {
      checkboxElement.checked = !checked; // Revert checkbox
      UI.showToast('보류 또는 중단 상태의 할 일은 체크박스로 변경할 수 없습니다', 'warning');
      return;
    }
